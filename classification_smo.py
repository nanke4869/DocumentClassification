#-*-coding:ISO-8859-1-*-

from numpy import *
import numpy as np


# ¶¨Òå´æ´¢±äÁ¿µÄÀà
class opStruct():
    def __init__(self, dataMatIn, classLabels, C, toler, kTup, kTup1):
        self.X = dataMatIn  # Êý¾Ý
        self.labelMat = classLabels  # ±êÇ©
        self.C = C  # ÈÝÈÌ¶È
        self.toler = toler  # Îó²îµÄÈÝÈÌ¶È
        self.m = shape(dataMatIn)[0]  # Êý¾ÝµÄ¸öÊý
        self.alphas = mat(zeros((self.m, 1)))  # alpha Öµ£¬Ã¿¸öÊý¾Ý¶ÔÓ¦Ò»¸öalpha
        self.b = 0  # ³£ÊýÏî
        self.eCache = mat(zeros((self.m, 2)))  # ±£´æÎó²îºÍÏÂ±í
        self.k = mat(zeros((self.m, self.m)))
        for i in range(self.m):
            self.k[:, i] = kernelTrans(self.X, self.X[i, :], kTup, kTup1)


def getrightfile(filename1, filename2):
    """
    ÔØÈëÊý¾Ý
    """
    a = np.load(filename1)
    b = np.load(filename2)
    a_float = map(float, a)
    b_float = map(float, b)
    dataMatIn = a_float.tolist()
    classLabels = b_float.tolist()
    return dataMatIn, classLabels


def kernelTrans(X, A, kTup, kTup1):
    """
    ¼ÓÈëºËº¯Êý
    """
    m, n = shape(X)
    k = mat(zeros((m, 1)))
    if kTup == 'lin':
        k = X * A.T
    elif kTup == 'rbf':
        for j in range(m):
            deltaRow = X[j, :] - A  # Ã¿Ò»ÐÐ¼õÈ¥A£¬ÔÚ×Ô¼º³Ë
            k[j] = deltaRow * deltaRow.T
        k = exp(k / (-1 * kTup1 ** 2))  # ¾ÍÊÇÀûÓÃµÄ¹«Ê½
    return k


def clipAlpha(ai, H, L):
    """
    ±£Ö¤alpha±ØÐëÔÚ·¶Î§ÄÚ
    """
    if ai > H:
        ai = H
    elif ai < L:
        ai = L
    return ai


def selectJrand(i, oS):
    """
    Ëæ»úÑ¡ÔñµÚ¶þ¸ö²»Í¬µÄalpha
    """
    j = i
    while i == j:
        j = int(np.random.uniform(0, oS.m))
    return j


def calcEk(oS, k):
    """
    ¼ÆËãÎó²î
    """
    fXk = float((multiply(oS.alphas, oS.labelMat)).T * oS.k[:, k] + oS.b)  # Ô¤²âÖµ
    Ek = fXk - oS.labelMat[k]  # Îó²îÖµ
    return Ek


def selectJ(i, oS, Ei):
    """
    Ñ¡ÔñµÚ¶þ¸öalpha ²¢ÇÒÏà²î×î´óµÄ
    """
    maxK = -1
    maxDelaE = 0
    Ej = 0
    oS.eCache[i] = [1, Ei]
    validEcaheList = nonzero(oS.eCache[:, 0].A)[0]
    if len(validEcaheList) > 0:
        for k in validEcaheList:
            if k == i:  # È¡²»Í¬µÄ alpha
                continue
            Ek = calcEk(oS, k)  # ¼ÆËãkµÄÓë²âÊÔÓëÕæÊµÖµÖ®¼äµÄÎó²î
            deltaE = abs(Ei - Ek)  # ÕÒÓëEi ¾àÀë×îÔ¶µÄ
            if maxDelaE < deltaE:
                maxDelaE = deltaE
                maxK = k  # ÓëEi²î±ð×î´óµÄK
                Ej = Ek  # KµÄÎó²î
        return maxK, Ej
    else:
        j = selectJrand(i, oS)
        Ej = calcEk(oS, j)  # ¼ÆËãÔ¤²âÖµºÍÕæÊµÖµµÄÎó²î
    return j, Ej


def updateEk(oS, k):
    """
    ¸üÐÂÎó²î
    """
    Ek = calcEk(oS, k)
    oS.eCache[k] = [1, Ek]


def innerL(i, oS):
    """
    SMO ÓÅ»¯
    """
    Ei = calcEk(oS, i)
    # ÔÚÎó²îÔÊÐíµÄ·¶Î§Íâ£¬Èç¹ûÐ¡ÓÚ¹æ¶¨µÄÎó²î£¬¾Í²»ÐèÒª¸üÐÂÁË
    if ((oS.labelMat[i] * Ei) <= oS.toler and oS.alphas[i] <= oS.C) or \
            ((oS.labelMat[i] * Ei) <= oS.toler and oS.alphas[i] >= 0):
        j, Ej = selectJ(i, oS, Ei)  # Ñ¡ÔñÁíÒ»¸öalphajºÍÔ¤²âÖµÓëÕæÊµÖµµÄ²î
        alphaIold = oS.alphas[i].copy()  # ¸´ÖÆalpha£¬ÒòÎªºó±ß»áÓÃµ½
        alphaJold = oS.alphas[j].copy()

        if (oS.labelMat[i] != oS.labelMat[j]):  # Á½¸öÀà±ð²»Ò»Ñù Ò»¸öÕýÀà Ò»¸ö¸ºÀà
            L = max(0, oS.labelMat[j] - oS.labelMat[i])  # Ô¼ÊøÌõ¼þ ²©¿ÍÀïÓÐ
            H = min(oS.C, oS.C + oS.alphas[j] - oS.alphas[i])
        else:
            L = max(0, oS.alphas[j] + oS.alphas[i] - oS.C)
            H = min(oS.C, oS.alphas[j] + oS.alphas[i])

        if L == H:
            print('L == H')
            return 0

        # ÀûÓÃºËº¯Êý
        eta = 2.0 * oS.k[i, j] - oS.k[i, i] - oS.k[j, j]
        if eta > 0:
            return 0
        oS.alphas[j] -= oS.labelMat[j] * (Ei - Ej) / eta  # ¾ÍÊÇ°´×îºóµÄ¹«Ê½Çó½â
        oS.alphas[j] = clipAlpha(oS.alphas[j], H, L)  # ÔÚL£¬H·¶Î§ÄÚ
        updateEk(oS, j)

        if (oS.alphas[j] - alphaJold) < 0.0001:
            return 0

        oS.alphas[i] += oS.labelMat[j] * oS.labelMat[i] * (alphaJold - oS.alphas[j])
        updateEk(oS, i)

        # ÀûÓÃºËº¯ÊýµÄ
        b1 = oS.b - Ei - oS.labelMat[i] * oS.k[i, i] * (oS.alphas[i] - alphaIold) - oS.labelMat[j] * oS.k[i, j] * (
                    oS.alphas[j] - alphaJold)
        b2 = oS.b - Ej - oS.labelMat[i] * oS.k[i, j] * (oS.alphas[i] - alphaIold) - oS.labelMat[j] * oS.k[j, j] * (
                    oS.alphas[j] - alphaJold)

        # ¸úÐÂb
        if oS.alphas[i] < oS.C and oS.alphas[i] > 0:
            oS.b = b1
        elif oS.alphas[j] < oS.C and oS.alphas[j] > 0:
            oS.b = b2
        else:
            oS.b = (b1 + b2) / 2.0
        return 1
    else:
        return 0


def calcWs(alpha, dataArr, classLabels):
    """
    ¼ÆËãalpha »ñµÃ·ÖÀàµÄÈ¨ÖØÏòÁ¿
    :param alpha:
    :param dataArr: ÑµÁ·Êý¾Ý
    :param classLabels: ÑµÁ·±êÇ©
    :return:
    """
    X = mat(dataArr)
    labelMat = mat(classLabels).T  # ±ä³ÉÁÐÏòÁ¿
    m, n = shape(X)
    w = zeros((n, 1))  # wµÄ¸öÊýÓë Êý¾ÝµÄÎ¬ÊýÒ»Ñù
    for i in range(m):
        w += multiply(alpha[i] * labelMat[i], X[i, :].T)  # alpha[i] * labelMat[i]¾ÍÊÇÒ»¸ö³£Êý  X[i,:]Ã¿£¨ÐÐ£©¸öÊý¾Ý£¬ÒòÎªwÎªÁÐÏòÁ¿£¬ËùÒÔÐèÒª×ªÖÃ
    return w


def smoP(dataMatIn, classLabels, C, toler, maxIter, kTup='lin', kTup1=0):
    """
    ºËÐÄÖ÷º¯Êý
    :param dataMatIn: ÑµÁ·Êý¾Ý
    :param classLabels: ÑµÁ·±êÇ©
    :param C: ³£Á¿
    :param toler:ÈÝ´í¶È
    :param maxIter: ×î´óµü´ú´ÎÊý
    :param kTup: ºËº¯ÊýÀàÐÍ²ÎÊý
    :param kTup1: ºËº¯ÊýÀàÐÍ²ÎÊý
    :return:
    """
    oS = opStruct(mat(dataMatIn), mat(classLabels).T, C, toler, kTup, kTup1)
    iter = 0
    entireSet = True
    alphaPairedChanged = 0
    while (iter < maxIter) and ((alphaPairedChanged > 0) or (entireSet)):
        alphaPairedChanged = 0
        if entireSet:
            # ±éÀúËùÓÐµÄÊý¾Ý ½øÐÐ¸üÐÂ
            for i in range(oS.m):
                alphaPairedChanged += innerL(i, oS)
            iter += 1
        else:
            nonBoundIs = nonzero((oS.alphas.A > 0) * (oS.alphas.A < oS.C))[0]
            for i in nonBoundIs:
                alphaPairedChanged += innerL(i, oS)
            iter += 1

        if entireSet:
            entireSet = False
        elif (alphaPairedChanged == 0):
            entireSet = True
    return oS.b, oS.alphas


if __name__ == '__main__':
    dataMatIn, classLabels = getrightfile('email_smo.npy', 'label_smo.npy')
    b, alphas = smoP(dataMatIn, classLabels, C=0.6, toler=0.001, maxIter=40, kTup='lin', kTup1=1)
    print("b:\n{}".format(b))
    print("alphas:\n{}".format(alphas))